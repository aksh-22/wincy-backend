...
import { CreateLeadDto, CreateLeadFromWebDto } from './dto/createLead.dto';
...
export class LeadsController {
  ...
  @Post('website/:orgId')
  async createLeadFromWebsite(
    @Body() dto: CreateLeadFromWebDto,
    @Param('orgId') orgId: string,
  ) {
    return await this.leadsService.createLeadFromWebSite(dto, orgId);
  }
  ...
  async updateLeadActivity(
    ...
    const { user } = req;
    return await this.leadsService.updateLeadActivity(
      user,
      orgId,
      dto,
      leadAct,
  ...
  async deleteLeadActivity(
    ...
    const { user } = req;
    return await this.leadsService.deleteLeadActivities(
      user,
      orgId,
      dto.activityIds,
    );
  ...
  @Roles(Role.Admin, Role['Member++'])
  async getFavouriteLeads(@Param('organisation') orgId: string) {
  ...
  @Roles(Role.Admin, Role['Member++'])
  async getPendingLeads(@Request() req, @Param('organisation') orgId: string) {
    const { user } = req;
