...
export class QueriesService {
  ...
  async updateQuery(user, orgId, projectId, queryId, attachments, dto) {
    ...
    if (attachments.length > 0 || dto.deleteAttachments?.length > 0) {
      this.utilsService.createAttachmentActivities(
        user._id,
        'Query',
        attachments,
  ...
  async deleteQueries(user, orgId, projectId, queries) {
    ...
    if (isProjectManager || ['Admin', 'Member++'].includes(user.type)) {
      queriesFull = await this.queryModel
        .find({ _id: { $in: queries }, project: projectId })
        .exec();
      deleted = await this.queryModel
        .deleteMany({ _id: { $in: queries }, project: projectId })
        .exec();
    } else {
      queriesFull = await this.queryModel
        .find({
    ...
    return {
      data: { deleted },
      success: true,
      message: 'Queries got deleted successfully.',
    };
  }
  ...
  async getQueries(user, orgId, projectId, status) {
    ...
    for (let i = 0; i < queries.length; i++) {
      queries[i].count = await this.replyModel
        .find({ query: queries[i]._id })
        .countDocuments()
        .exec();
    }
  ...
  async getReplies(user, orgId, projectId, queryId) {
    ...
    const query = await this.queryModel
      .findOne({ _id: queryId, project: projectId })
      .exec();
  ...
  async deleteReplies(user, orgId, projectId, replies) {
    ...
    if (isProjectManager || ['Admin', 'Member++'].includes(user.userType)) {
      deletedDetailed = await this.replyModel
        .find({ _id: { $in: replies } })
        .exec();
      deleted = await this.replyModel
        .deleteMany({ _id: { $in: replies } })
        .exec();
    } else {
      deletedDetailed = await this.replyModel
        .find({ _id: { $in: replies }, createdBy: user._id })
        .exec();
      deleted = await this.replyModel
        .deleteMany({ _id: { $in: replies }, createdBy: user._id })
        .exec();
    }
