...
export class EventsService {
  ...
  async createPublicEvents(user, orgId, dto) {
    ...
    if (user.type == 'Member+' && user.email != 'info@pairroxz.com') {
      //Extra permission given to info Pairroxz Id, should be removed in future.
      const projectManagers = project.projectManagers.map((el) => String(el));
      if (dto.project !== undefined && dto.category == 'Meeting') {
        project = await this.projectsService.getAppProject(
          { _id: dto.project, organisation: orgId },
          {},
        );
        if (!project && projectManagers.includes(String(user._id))) {
        ...
        throw new HttpException(
          'Member+ is authorised to add project level meetings only!',
          HttpStatus.BAD_REQUEST,
        );
      }
    }
    if (dto.range === true && dto.dateFrom && dto.dateTo && !dto.date) {
      throw new HttpException(
        'Please provide date range parameters!',
        HttpStatus.BAD_REQUEST,
      );
    }
    ...
    let event = new this.eventModel({
      ...
      organisation: orgId,
      project: ['Meeting', 'Milestone'].includes(dto.category)
        ? dto.project
        : undefined,
  ...
  async updatePublicEvent(user, orgId, eventId, dto) {
    let event = await this.eventModel
      .findOne({ organisation: orgId, _id: eventId })
      .exec();
    if (!event) {
    ...
    if (user.type == 'Member+' && user.email != 'info@pairroxz.com') {
      if (
        (dto.project !== undefined || event.project !== undefined) &&
    ...
    if (
      dto.isRange === true ||
      (dto.isRange === undefined && event.isRange === true)
    ) {
      if (dto.dateTo) {
    ...
    if (dto.isRange === false) {
      ...
      if (!dto.date) {
        throw new HttpException(
          'Please, provide date of event in case of no range of dates.',
          HttpStatus.BAD_REQUEST,
        );
      }
    }
  ...
  async getPublicEvents(user, orgId, month, year) {
    ...
    if (['Admin', 'Member++'].includes(user.type)) {
      ...
      const projects = await this.projectsService.getMultiProjectsApp(
        {
          organisation: orgId,
        ...
        $or: [
          {
            type: 'Public',
            category: 'Milestone',
            organisation: mongoose.Types.ObjectId(orgId),
  ...
  async deletePublicEvent(user, orgId, eventId) {
    ...
    if (user.type == 'Member+') {
      ...
      deleted = await this.eventModel
        .deleteOne({ type: 'Public', organisation: orgId, _id: eventId })
        .exec();
    }
    if (deleted.n < 1) {
      throw new HttpException(
        'Event did not get deleted! Please try again.',
        HttpStatus.BAD_REQUEST,
      );
    }
    return {
  ...
  async updatePrivateEvent(user, eventId, dto) {
    let event = await this.eventModel
      .findOne({ _id: eventId, type: 'Private' })
  ...
  async deletePrivateEvent(user, eventId) {
    const deleted = await this.eventModel
      .deleteOne({ type: 'Private', createdBy: user._id, _id: eventId })
      .exec();
    if (deleted.n < 1) {
      throw new HttpException(
        'Event did not get deleted!',
        HttpStatus.BAD_REQUEST,
      );
    }
    return {
  ...
  async getMyEvents(user, orgId, month, year) {
    const publicEvents = (await this.getPublicEvents(user, orgId, month, year))
      .data.events;
    const privateEvents = (await this.getPrivateEvents(user, month, year)).data
      .events;

    return {
      data: { publicEvents: publicEvents, privateEvents: privateEvents },
      message: '',
      success: true,
    };
  }
