...
export class BugsService {
  ...
  async deleteBugs(user, bugs, projectId, orgId) {
    const project = await this.projectsService.getAppProject(
      { _id: projectId },
      { organisation: 1, projectManagers: 1 },
    );
    if (String(project.organisation) != orgId) {
      throw new HttpException(
        "Given set of project and organisation doen't match up!",
        HttpStatus.BAD_REQUEST,
      );
    }
    let filter;
    ...
    return {
      success: true,
      message: 'Bugs got deleted successfully.',
      data: {},
    };
  }
  ...
  async createBugReport(user, dto, projectId) {
    ...
    const bugs = await this.bugModel
      .find({ isCompleted: true, pickedBy: user._id, completedOn: dateToday })
      .exec();
    if (bugs.length == 0) {
      new HttpException(
        'Please, mark your bugs as completed before submitting report',
        HttpStatus.BAD_REQUEST,
      );
    }
    if (dto.platform) {
  ...
  async updateBugReport(user, dto, bugReportId) {
    ...
    const project = await this.projectsService.getAppProject(
      { _id: bugReport.project },
      {},
    );
    if (!bugReport || String(user._id) != String(bugReport.createdBy)) {
      throw new HttpException(
        "Given Set of BugReport and User doesn't exists!",
        HttpStatus.BAD_REQUEST,
      );
    }
    project.consumedTime =
      project.consumedTime - bugReport.consumedTime + dto.consumedTime;
  ...
  async getSingleBug(bugId, projectId, orgId) {
    ...
    if (project.organisation != orgId) {
      throw new HttpException(
        'Given project is not associated with the provided organisation!',
        HttpStatus.BAD_REQUEST,
      );
    }
  ...
  async addBugAttachments(user, bugId, attachments) {
    ...
    if (
      ...
      if (bug.attachments) {
        bug.attachments.push(...proAttach);
      } else {
        bug.attachments = [...proAttach];
      }
      await bug.save();
    ...
    return {
      message: 'Files successfully added to the Bug.',
      data: { attachments: bug.attachments },
      success: true,
    };
  }
  ...
  async removeBugAtachments(user, bugId, attachments) {
    ...
    return {
      message: 'Files sucessfully deleted',
      success: true,
      data: { attachments: bug.attachments },
    };
  }
  ...
  async getBugsForTask(user, orgId, projectId, taskId) {
    ...
    const bugs = await this.getBugPopulated({
      task: mongoose.Types.ObjectId(taskId),
    });
